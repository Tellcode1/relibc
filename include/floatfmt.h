...
#define LDBL_EXPONENT_OFFSET (1 + LDBL_EXPONENT_SIZE)
  ...
  struct float_repr
  {
    ...
    int64_t  exponent : (FLT_EXPONENT_SIZE + 1); // unbiased raw bits + sign bit (it's unbiased so we need to include the sign bit)
  ...
  {
    uint8_t  sign : (1);                         // 0 -> positive, 1 -> negative
    int64_t  exponent : (DBL_EXPONENT_SIZE + 1); // unbiased raw bits + sign bit (it's unbiased so we need to include the sign bit)
  ...
  {
    uint8_t  sign : (1);
    int64_t  exponent : (LDBL_EXPONENT_SIZE + 1); // unbiased raw bits + sign bit (it's unbiased so we need to include the sign bit)
...
#define LDBL_EXP_BIAS (LDBL_MAX_EXP - 1)
  ...
  static inline RE_ATTR_CONST struct float_repr
  re_break_float(float_t flt)
  {
    ...
    if (raw_exp != 0) { repr.mantissa |= 1ULL << 23; }
    return repr;
  }

  static inline RE_ATTR_CONST struct double_repr
  ...
  static inline RE_ATTR_CONST struct long_double_repr
